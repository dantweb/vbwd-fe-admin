<template>
  <div class="cms-list">
    ...
    <div
      v-if="store.selectedLayoutIds.size > 0"
    ...
    <table class="cms-table">
      ...
      <tbody>
        <tr
          v-for="layout in items"
          :key="layout.id"
          class="cms-table__row"
          @click.self="$router.push(`/admin/cms/layouts/${layout.id}/edit`)"
        >
          <td @click.stop>
            <input
              type="checkbox"
              :checked="store.selectedLayoutIds.has(layout.id)"
              @change="toggleOne(layout.id)"
            >
          </td>
          <td @click="$router.push(`/admin/cms/layouts/${layout.id}/edit`)">
            {{ layout.name }}
          </td>
          <td>{{ layout.slug }}</td>
          <td>
...
const items = computed(() => store.layouts?.items ?? []);
const totalPages = computed(() => store.layouts?.pages ?? 1);
const allSelected = computed(() =>
  items.value.length > 0 && items.value.every(l => store.selectedLayoutIds.has(l.id))
);
...
function fetch() {
  store.fetchLayouts({ page: page.value, query: query.value, sort_by: sortBy.value, sort_dir: sortDir.value });
}
...
function toggleAll() {
  if (allSelected.value) items.value.forEach(l => store.selectedLayoutIds.delete(l.id));
  else items.value.forEach(l => store.selectedLayoutIds.add(l.id));
}

function toggleOne(id: string) {
  if (store.selectedLayoutIds.has(id)) store.selectedLayoutIds.delete(id);
  else store.selectedLayoutIds.add(id);
}
...
.area-chip { display: inline-block; font-size: 0.7rem; padding: 2px 8px; border-radius: 10px; background: var(--admin-badge-ok-bg, #d4edda); color: var(--admin-badge-ok, #155724); margin-right: 3px; white-space: nowrap; }
